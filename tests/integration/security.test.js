...
import { setupTests, createTestUser, testEmail } from '../setup/test-helpers.js';
...
describe('Security', () => {
  ...
  test('Unicode and special characters handled safely', async () => {
    const specialChars = "测试 émojis 🎉 tabs\t\nnewlines\r\n quotes'\"backslash\\";
  ...
  test.skip('TODO: Null byte attack prevented', async () => {
    // Null bytes cause PostgreSQL NOTIFY errors
    const nullByteAttack = "test\x00.txt";
    ...
    expect(result[0].item.id).toBeDefined();
  });

  test.skip('TODO: Extremely long input handled', async () => {
    // Very long strings cause NOTIFY payload size errors
    ...
    expect(result[0].item.description.length).toBe(10000);
  });
