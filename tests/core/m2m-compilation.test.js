...
test("Compiler generates M2M sync logic WITHOUT runtime loops", () => {
  ...
  expect(result.sql).toContain(
    "FROM resource_tags WHERE resource_id = v_result.id",
  );
  ...
  expect(result.sql).not.toContain(
    "jsonb_object_keys(l_entity_config.many_to_many)",
  );
  expect(result.sql).not.toContain("SELECT jsonb_object_keys(");
  expect(result.sql).not.toMatch(
    /FOR.*IN.*SELECT.*jsonb_object_keys.*many_to_many/,
  );
...
test("GET function includes M2M fields", () => {
  ...
  const getFunctionMatch = result.sql.match(
    /CREATE OR REPLACE FUNCTION get_brands\([\s\S]*?\$\$ LANGUAGE plpgsql/,
  );
...
test("SEARCH function includes M2M fields via LATERAL joins", () => {
  ...
  const searchFunctionMatch = result.sql.match(
    /CREATE OR REPLACE FUNCTION search_resources\([\s\S]*?\$\$ LANGUAGE plpgsql/,
  );
  ...
  expect(searchFunction).toContain(
    "JOIN users target ON target.id = jt.user_id",
  );
