...
describe('SQL Validation Tests', () => {
  ...
  describe('Generated SQL Structure', () => {
    ...
    test('generates graph rule functions', () => {
      expect(compiledSQL).toContain('CREATE OR REPLACE FUNCTION graph_organisations_on_create');
    });
  ...
  describe('GET Function', () => {
    test('has correct signature', () => {
      expect(compiledSQL).toMatch(/CREATE OR REPLACE FUNCTION get_organisations\(\s*p_id INT,\s*p_user_id INT/);
  ...
  describe('SAVE Function', () => {
    test('has correct signature', () => {
      expect(compiledSQL).toMatch(/CREATE OR REPLACE FUNCTION save_organisations\(\s*p_data JSONB,\s*p_user_id INT/);
    ...
    test('includes graph rules call', () => {
      ...
      expect(saveFunction).toContain('graph_organisations_on_create');
    });
  ...
  describe('DELETE Function', () => {
    test('has correct signature', () => {
      expect(compiledSQL).toMatch(/CREATE OR REPLACE FUNCTION delete_organisations\(\s*p_id INT,\s*p_user_id INT/);
    });
  ...
  describe('Graph Rules', () => {
    test('has correct signature', () => {
      expect(compiledSQL).toMatch(/CREATE OR REPLACE FUNCTION graph_organisations_on_create\(\s*p_record JSONB,\s*p_user_id INT/);
      expect(compiledSQL).toMatch(/graph_organisations_on_create.*RETURNS VOID/s);
    });

    test('includes comment describing the rule', () => {
      const graphFunction = compiledSQL.match(/CREATE OR REPLACE FUNCTION graph_organisations_on_create.*?END;/s)[0];
