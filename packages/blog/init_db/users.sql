...
-- Permission check: view on users (public access)
CREATE OR REPLACE FUNCTION can_view_users(
  p_user_id INT,
  p_record JSONB
) RETURNS BOOLEAN AS $$
BEGIN
...
-- Permission check: update on users
CREATE OR REPLACE FUNCTION can_update_users(
  p_user_id INT,
  p_record JSONB
) RETURNS BOOLEAN AS $$
BEGIN
...
-- Permission check: delete on users
...
BEGIN
  ...
  ELSE
    ...
    IF (SELECT COUNT(*) FROM jsonb_object_keys(p_data) WHERE jsonb_object_keys != 'id') > 0 THEN
      EXECUTE (
        SELECT format(
          'UPDATE users SET %s WHERE id = %L RETURNING *',
...
BEGIN
  ...
  IF p_search IS NOT NULL THEN
    v_where_clause := v_where_clause || ' AND (name ILIKE '%' || p_search || '%' OR email ILIKE '%' || p_search || '%')';
  END IF;

  -- Add temporal filter

  -- Get total count
  EXECUTE format('SELECT COUNT(*) FROM users WHERE %s', v_where_clause) INTO v_total;
...
-- ============================================================================
-- Auth: _profile function for users
...
-- ============================================================================
-- Auth: register_user function for users
...
-- ============================================================================
-- Auth: login_user function for users
-- ============================================================================
